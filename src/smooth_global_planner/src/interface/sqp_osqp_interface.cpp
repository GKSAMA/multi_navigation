...
bool SqpOsqpInterface::Solve() {
    ...
    if (ref_points_.size() != bounds_around_refs_.size()) {
        LOG(ERROR) << "ref_points and bounds size not equal, solver early terminates";
        return false;
    }
    ...
    if (!initial_solve_res) {
        LOG(ERROR) << "initial iteration solving fails";
        osqp_cleanup(work);
        c_free(data->A);
        c_free(data->P);
        c_free(data);
    ...
    while (pen_itr < sqp_pen_max_iter_) {
        ...
        if (!iterative_solve_res) {
            LOG(ERROR) << "iteration at " << sub_itr
        ...
        if (ftol < sqp_ftol_) {
            // LOG(INFO) << "merit function value converges at sub itr num " << sub_itr;
            // LOG(INFO) << "merit function value converges to " << cur_fvalue
        ...
        if (!fconverged) {
            LOG(ERROR) << "Max number of iteration reached";
            weight_curvature_constraint_slack_var_ = original_slack_penalty;
            osqp_cleanup(work);
            c_free(data->A);
            c_free(data->P);
        ...
        // LOG(INFO) << "ctol is " << ctol << ", at pen itr " << pen_itr;

        if (ctol < sqp_ctol_) {
            // LOG(INFO) << "constraint satisfied at pen itr num " << pen_itr;
            // LOG(INFO) << "constraint voilation value drops to " << ctol
    ...
    // LOG(INFO) << "constraint not satisfied with total itr num " << pen_itr;
    // LOG(INFO) << "constraint voilation value drops to " << ctol
...
void SqpOsqpInterface::CalculateKernel(
...
void SqpOsqpInterface::CalculateOffset(std::vector<c_float>* q) {
...
std::vector<double>
SqpOsqpInterface::CalculateLinearizedFemPosParams(
...
void SqpOsqpInterface::CalculateAffineConstraint(
...
void SqpOsqpInterface::SetPrimalWarmStart(
    const std::vector<std::pair<double, double>>& points,
    std::vector<c_float>* primal_warm_start) {
...
bool SqpOsqpInterface::OptimizeWithOsqp(
...
double SqpOsqpInterface::CalculateConstraintViolation(
    const std::vector<std::pair<double, double>>& points) {
