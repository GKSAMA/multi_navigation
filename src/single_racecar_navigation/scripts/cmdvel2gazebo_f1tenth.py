...
class CmdVel2Gazebo:
    ...
    def __init__(self,namespace):
        ...
        self.pub_steerL = rospy.Publisher(namespace+'/front_left_steering_position_controller/command', Float64, queue_size=1)
        self.pub_steerR = rospy.Publisher(namespace+'/front_right_steering_position_controller/command', Float64, queue_size=1)
        self.pub_frontL = rospy.Publisher(namespace+'/front_left_velocity_controller/command', Float64, queue_size=1)
        self.pub_frontR = rospy.Publisher(namespace+'/front_right_velocity_controller/command', Float64, queue_size=1)
        self.pub_rearL = rospy.Publisher(namespace+'/rear_left_velocity_controller/command', Float64, queue_size=1)
        self.pub_rearR = rospy.Publisher(namespace+'/rear_right_velocity_controller/command', Float64, queue_size=1)
    ...
    def publish(self):
        ...
        if self.z != 0:
            ...
            # print("rL_front = ", rL_front, "  rR_front = ", rR_front,"rL_rear = ", rL_rear, "  rR_rear = ", rR_rear)
            msgRearR = Float64()
            msgFrontR = Float64()
            ...
            self.pub_steerL.publish(msgSteerL.data)
    
            # the right tire's angle is solved directly from geometry
            msgSteerR.data = math.atan2(L,rR_rear)*math.copysign(1,self.z)
            # print("msgSteerL = ", msgSteerL.data, "  msgSteerR = ", msgSteerR.data)
            self.pub_steerR.publish(msgSteerR.data)
        else:
