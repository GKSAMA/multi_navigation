...
class CmdVel2Gazebo:
    def __init__(self,namespace):
        ...
        rospy.Subscriber(namespace+'/cmd_vel', Twist, self.callback, queue_size=1)
        self.pub_steerL = rospy.Publisher(namespace+'/left_steering_hinge_position_controller/command', Float64, queue_size=1)
        self.pub_steerR = rospy.Publisher(namespace+'/right_steering_hinge_position_controller/command', Float64, queue_size=1)
        self.pub_frontL = rospy.Publisher(namespace+'/left_front_wheel_velocity_controller/command', Float64, queue_size=1)
        self.pub_frontR = rospy.Publisher(namespace+'/right_front_wheel_velocity_controller/command', Float64, queue_size=1)
        
        self.pub_rearL = rospy.Publisher(namespace+'/left_rear_wheel_velocity_controller/command', Float64, queue_size=1)
        self.pub_rearR = rospy.Publisher(namespace+'/right_rear_wheel_velocity_controller/command', Float64, queue_size=1)
        ...
        # car Tread
        # self.T_front = 0.14605
        # self.T_rear = 0.14605
        # self.T_front = 0.365 + 0.0889*2
        # self.T_rear = 0.365 + 0.0889*2
        self.T_front = 0.365 + 0.07*2
        self.T_rear = 0.365 + 0.07*2
    ...
    def publish(self):
        ...
        if self.z != 0:
            ...
            rL_front = math.sqrt(math.pow(L,2) + math.pow(rL_rear, 2))
            rR_front = math.sqrt(math.pow(L,2) + math.pow(rR_rear, 2))
            msgRearR = Float64()
            msgFrontR = Float64()
            ...
            msgFrontR.data = self.x*rR_front/r;
            msgRearL = Float64()
            msgFrontL = Float64()
            ...
            msgFrontL.data = self.x*rL_front/r;
            # print("msgRearL = " + str(msgRearL))
            # print("msgRearR = " + str(msgRearR))
            ...
            msgSteerL.data = math.atan2(L,rL_rear)*math.copysign(1,self.z)
            self.pub_steerL.publish(msgSteerL)

            # the right tire's angle is solved directly from geometry
            msgSteerR.data = math.atan2(L,rR_rear)*math.copysign(1,self.z)
