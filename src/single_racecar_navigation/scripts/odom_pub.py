...
pub_name    = "odom"
imu_name    = "imu"
joint_states_name   = "joint_states"
# 定义起始坐标
start_x     = -0.5
start_y     = 0
start_z     = 0
class odom_class:
    ...
    #velocity = 0
    namespace = ""
    def __init__(self, ns, x, y, z, th):
        self.namespace = ns
        self.start_x = float(x)
        self.start_y = float(y)
        self.start_z = float(z)
        self.yaw_angle = float(th)
        # 订阅 IMU 传感器（陀螺仪）
        rospy.Subscriber(self.namespace + "/" + imu_name, Imu, self.callback_imu, queue_size=1)
        # 订阅 关节
        rospy.Subscriber(self.namespace + "/" + joint_states_name, JointState, self.callback_join, queue_size=1)
        # 发布 odom 坐标
        self.pub = rospy.Publisher(self.namespace + "/" + pub_name, Odometry, queue_size = 1)
        ...
        self.odom_topic.header.frame_id = self.namespace + "/" + frame_name
        self.odom_topic.child_frame_id = self.namespace + "/" + child_frame_name
    ...
    def callback_join(self, data):
        ...
        # print("velocity = ",velocity, "add_time_joint = ",add_time_joint," position = ", position) 
        #  xy轴坐标(位置，必须)
        ...
        # print("cur pos x = ", self.odom_topic.pose.pose.position.x, "  cur pos y = ", self.odom_topic.pose.pose.position.y)
...
def main():
    ...
    node = odom_class(args.namespace,args.x, args.y, args.z, args.th)
    rospy.spin()
