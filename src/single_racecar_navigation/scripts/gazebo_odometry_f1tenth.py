...
class OdometryNode:
    ...
    def sub_robot_pose_update(self, msg):
        ...
        try:
            arrayIndex = msg.name.index(self.namespace[1:]+'::base_link')
            leftRearIndex = msg.name.index(self.namespace[1:]+'::rear_left_wheel_link')
            ...
            rear_pose.pose.position.x = self.last_received_pose.position.x - math.cos(yaw) * self.rear_to_footprint;
            rear_pose.pose.position.y = self.last_received_pose.position.y - math.sin(yaw) * self.rear_to_footprint;
    ...
    def timer_callback(self, event):
        ...
        cmd.child_frame_id = self.namespace+'/base_link'
